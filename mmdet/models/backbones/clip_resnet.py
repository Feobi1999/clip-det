...
class CLIPResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim=512, input_resolution=224, width=64, pretrained=None, frozen_stages=-1, **kwargs):
        super().__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        visual_pth=pretrained[1]
        if isinstance(visual_pth, str):
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def forward(self, text, context):
        import pdb
        # pdb.set_trace()
        x_text = self.token_embedding(text)  # n_clas, n_text, C   #80,5,512
        K, N1, C = x_text.shape
